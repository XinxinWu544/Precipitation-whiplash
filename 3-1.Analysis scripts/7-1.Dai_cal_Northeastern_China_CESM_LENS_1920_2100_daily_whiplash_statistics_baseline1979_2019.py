...
def cal_whiplash_events(j):
    ...
    d_to_w=np.vstack((dw_d_end,cumu_start,dw_w_start,cumu_end)).T
...
def cal_anom_quantile_Time_varying(var):
    ...
    day_cycle=pd.Series(cum_prec[:,j,k]).groupby(calendar_day).mean()
    #sd=pd.Series(cum_prec[:,j,k]).groupby(calendar_day).std()
    anom_prec= cum_prec[:,j,k]-np.tile(day_cycle,181)
...
def rough_event_Daily_SPI_proxy(j):
    ...
    dry_events=(a > np.tile(b.sel(quantile= round(1-q,2)).values,181) ).astype(float) #找出事件
    c = dry_events.cumsum()
    c[np.where(np.array(dry_events)!=0)]=np.nan
    c[0:(min_period-1)]=np.nan
    

    wet_events=(a < np.tile(b.sel(quantile= q).values,181) ).astype(float) 
    d = wet_events.cumsum()
    d[np.where(np.array(wet_events)!=0)]=np.nan
    d[0:(min_period-1)]=np.nan
    return {'dry_events':c,'wet_events':d}
...
#%%
n=1
result_dir = basic_dai_dir +'code/7-1.CESM_LENS_daily_whiplash_stats_baseline_40_ensemble/'  #硬盘不够位置了，拷在本机
...
for plan in [1]:
    ...
    for n in num:
        ...
        #2. 读入降水数据
        data=xr.open_dataarray('/media/dai/DATA2/CESM-LENS/PRECT/PRECT_'+str(n).zfill(3)+'_interp2deg.nc' ).sel(lon=slice(120,130),lat=slice(50,40))
        ...
        for dtrd_typ in [Dtrd_typ]: #不去趋势 #linear #二阶趋势 #多模型平均进行缩放
            ...
            if dtrd_typ in [2,3,4]:
                ...
                data_year=xr.open_dataarray(basic_dir + 'code/4-0.CESM_LENS_1920_2100_ensemble_annual_mean_prec/PRECT_annual_mean_'+str(n).zfill(3)+'.nc').sel(lon=slice(120,130),lat=slice(50,40))
                
                data_year_dtrd=xr.open_dataarray(basic_dir + 'code/4-0.CESM_LENS_1920_2100_ensemble_annual_mean_prec/PRECT_annual_mean_'+str(n).zfill(3)+'_detrend'+str(dtrd_typ-1)+'.nc').sel(lon=slice(120,130),lat=slice(50,40))
                ...
                data2 = data1.weighted(np.cos(np.deg2rad(data1.lat) ) ).mean(('lon','lat'))
            ...
            for min_period in [Min_period]:
                ...
                cum_prec=data2.rolling(time=min_period,center=False).sum()
                cum_prec=cum_prec.assign_coords({'time':calendar_day})
                
                a =np.zeros(shape=(66065,1,1))
                a[:,0,0] = cum_prec.values
                cum_prec= xr.DataArray( a,dims=('time','lat','lon'),coords={'time':cum_prec.time,'year':cum_prec.year,'lon':[125],'lat':[35]} )
