...
for plan in [1]:
    ...
    for ex in ['dry_to_wet','wet_to_dry']  :
        ...
        for n in num:
            ...
            method_dir= basic_dai_dir +'code_new/6-4.CESM_LENS_daily_whiplash_stats_baseline_40_ensemble_new_intensity/'+str(n).zfill(3)+'/'
        ...
        events_counts.to_netcdf(dai_basic_dir + 'code_new/8-2.CESM_LENS_event_frequency_duration_intensity_all_new_intensity/frequency_40ensemble'+
                                '_'+ex+'_'+thes_typ+'_detrend_'+str(dtrd_typ)+'_of_'+str(min_period)+'_days'+
                                    '_quantile_'+str(q)+'_inter_period_'+str(inter_period)+'.nc')
        
        events_duration.to_netcdf(dai_basic_dir + 'code_new/8-2.CESM_LENS_event_frequency_duration_intensity_all_new_intensity/duration_40ensemble'+
                                '_'+ex+'_'+thes_typ+'_detrend_'+str(dtrd_typ)+'_of_'+str(min_period)+'_days'+
                                    '_quantile_'+str(q)+'_inter_period_'+str(inter_period)+'.nc')
          
        events_intensity.to_netcdf(dai_basic_dir + 'code_new/8-2.CESM_LENS_event_frequency_duration_intensity_all_new_intensity/intensity_40ensemble'+
                                '_'+ex+'_'+thes_typ+'_detrend_'+str(dtrd_typ)+'_of_'+str(min_period)+'_days'+
                                    '_quantile_'+str(q)+'_inter_period_'+str(inter_period)+'.nc')
         
        events_severity.to_netcdf(dai_basic_dir + 'code_new/8-2.CESM_LENS_event_frequency_duration_intensity_all_new_intensity/severity_40ensemble'+
                                '_'+ex+'_'+thes_typ+'_detrend_'+str(dtrd_typ)+'_of_'+str(min_period)+'_days'+
                                    '_quantile_'+str(q)+'_inter_period_'+str(inter_period)+'.nc')
