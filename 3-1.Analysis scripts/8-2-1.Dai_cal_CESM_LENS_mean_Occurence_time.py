...
for plan in [1]:
    ...
    for ex in ['dry_to_wet','wet_to_dry']  :
        ...
        Time_Stats = list(tqdm.tqdm(pool.imap( cal_event_time, range(16200)), 
                                       total=16200, position=0, leave=True))
        pool.close()     
        ...
        events_current_time.to_netcdf(dai_basic_dir + 'code_new/8-2.CESM_LENS_event_frequency_duration_intensity_all_new_intensity/current_time_40ensemblemean'+
                                '_'+ex+'_'+thes_typ+'_detrend_'+str(dtrd_typ)+'_of_'+str(min_period)+'_days'+
                                    '_quantile_'+str(q)+'_inter_period_'+str(inter_period)+'.nc')
        events_future_time.to_netcdf(dai_basic_dir + 'code_new/8-2.CESM_LENS_event_frequency_duration_intensity_all_new_intensity/future_time_40ensemblemean'+
                                '_'+ex+'_'+thes_typ+'_detrend_'+str(dtrd_typ)+'_of_'+str(min_period)+'_days'+
                                    '_quantile_'+str(q)+'_inter_period_'+str(inter_period)+'.nc') 
