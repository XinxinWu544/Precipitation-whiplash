...
#%%
n=1
result_dir = basic_dai_dir +'code/5-1.CESM_LENS_daily_whiplash_stats_baseline_sensitivity/'  #硬盘不够位置了，拷在本机
...
for plan in [9,10]:
    ...
    for n in [1,5,9,13,17]:
        ...
        #2. 读入降水数据
        data=xr.open_dataarray('/media/dai/DATA2/CESM-LENS/PRECT/PRECT_'+str(n).zfill(3)+'_interp2deg.nc',chunks={'time':1000} )#.sel(lon=slice(10,30) )
        ...
        for dtrd_typ in [Dtrd_typ]: #不去趋势 #linear #二阶趋势 #多模型平均进行缩放
            ...
            for min_period in [Min_period]:
                ...
                cum_prec=data1.rolling(time=min_period,center=False).sum().compute()
                cum_prec=cum_prec.assign_coords({'time':calendar_day})
                
                cum_prec_baseline=cum_prec[np.where( (year <= 2019) & (year>=1979) )[0],:,:]
                calendar_day_baseline = [calendar_day[x] for x in np.where( (year <= 2019) & (year>=1979) )[0] ]
