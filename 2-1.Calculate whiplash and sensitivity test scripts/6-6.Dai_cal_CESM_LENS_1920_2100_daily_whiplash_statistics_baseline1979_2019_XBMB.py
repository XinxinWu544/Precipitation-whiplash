...
n=1
result_dir = basic_dai_dir +'code_new/6-6.CESM_SF_daily_whiplash_stats_baseline_new_intensity/'  #硬盘不够位置了，拷在本机
    
#%%


Sd = xr.open_dataarray('/media/dai/disk2/suk/research/4.East_Asia/Again/code_new/6-1.CESM_LENS_cumprec_sd_quan/LENS_40ensembles_sd.nc').mean('ensemble')
Day_cycle = xr.open_dataarray('/media/dai/disk2/suk/research/4.East_Asia/Again/code_new/6-1.CESM_LENS_cumprec_sd_quan/LENS_40ensembles_day_cycle.nc').mean('ensemble')
Quan_prec = xr.open_dataarray('/media/dai/disk2/suk/research/4.East_Asia/Again/code_new/6-1.CESM_LENS_cumprec_sd_quan/LENS_40ensembles_quan_prec.nc').mean('ensemble')
...
for plan in [1]:
    ...
    for n in range(num_forcing[x]):
        ...
        if os.path.exists(result_dir+'/X'+forcing[x]+'/'+str(n).zfill(3)+'/')==False:
            os.makedirs(result_dir+'/X'+forcing[x]+'/'+str(n).zfill(3)+'/')
        method_dir=result_dir+'/X'+forcing[x]+'/'+str(n).zfill(3)+'/'
        ...
        data=xr.open_dataarray('/media/dai/DATA2/CESM-LENS/PRECT/X'+forcing[x]+'/X'+forcing[x]+'_'+str(n).zfill(3)+'_interp2deg.nc' )#.sel(lon=slice(10,30) )
        ...
        for dtrd_typ in [Dtrd_typ]: #不去趋势 #linear #二阶趋势 #多模型平均进行缩放
            ...
            if dtrd_typ in [2,3,4]:
                ...
                data_year=xr.open_dataarray(basic_dir + 'code_new/4-3.CESM_SF_ensemble_annual_mean_prec/X'+forcing[x]+'_annual_mean_'+str(n).zfill(3)+'.nc')
                
                data_year_dtrd=xr.open_dataarray(basic_dir + 'code_new/4-3.CESM_SF_ensemble_annual_mean_prec/X'+forcing[x]+'_annual_mean_'+str(n).zfill(3)+'_detrend'+str(dtrd_typ-1)+'.nc')
