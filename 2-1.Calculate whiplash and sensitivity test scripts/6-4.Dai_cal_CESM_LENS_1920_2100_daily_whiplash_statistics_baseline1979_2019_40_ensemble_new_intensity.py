...
#%%
n=1
result_dir = basic_dai_dir +'code_new/6-4.CESM_LENS_daily_whiplash_stats_baseline_40_ensemble_new_intensity/'  #硬盘不够位置了，拷在本机
...
Sd = xr.open_dataarray(basic_dai_dir +'code_new/6-1.CESM_LENS_cumprec_sd_quan/LENS_40ensembles_sd.nc').mean('ensemble')
Day_cycle = xr.open_dataarray(basic_dai_dir +'code_new/6-1.CESM_LENS_cumprec_sd_quan/LENS_40ensembles_day_cycle.nc').mean('ensemble')
Quan_prec = xr.open_dataarray(basic_dai_dir +'code_new/6-1.CESM_LENS_cumprec_sd_quan/LENS_40ensembles_quan_prec.nc').mean('ensemble')
    ...
    for n in range(1,8):
        ...
        for dtrd_typ in [Dtrd_typ]: #不去趋势 #linear #二阶趋势 #多模型平均进行缩放
            ...
            if dtrd_typ in [2,3,4]:
                ...
                data_year_dtrd=xr.open_dataarray(basic_dir + 'code/4-7.CESM_LENS_1920_2100_ensemble_annual_mean_prec/PRECT_annual_mean_'+str(n).zfill(3)+'_detrend'+str(dtrd_typ-1)+'.nc')
