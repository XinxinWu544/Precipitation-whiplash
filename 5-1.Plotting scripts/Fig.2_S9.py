...
global_map = np.load(basic_dir+'code_whiplash/4-2.Input data for plotting/2_S9.LENS_change_map_of_features.npy',allow_pickle=True).tolist()
...
for ex in ['dry_to_wet','wet_to_dry']:
    ...
    for feature in ['frequency','duration','intensity']:
        for type in ['mean change','distribution']:
            ...
            if p_num in [2,4,6]:
                ...
                #1.LENS global ensemble mean 
                a = global_land_mean.get(ex+'~'+feature+'~'+'global-LENS').mean('ensemble')
                ax.plot(a.year,a,label='CESM-LENS-global',linewidth=1)
                
                a1= global_land_mean.get(ex+'~'+feature+'~'+'global-LENS').quantile(dim='ensemble',q=0.05)
                a2= global_land_mean.get(ex+'~'+feature+'~'+'global-LENS').quantile(dim='ensemble',q=0.95)
                
                ax.fill_between(x=a1.year,y1=a1,y2=a2,alpha=.3,color=color_d_type[0])
            
                #1.LENS land ensemble mean 
                a = global_land_mean.get(ex+'~'+feature+'~'+'land-LENS').mean('ensemble')
                ax.plot(a.year,a,label='CESM-LENS-land',linewidth=1,linestyle='--',color=color_d_type[0])
                
                a1= global_land_mean.get(ex+'~'+feature+'~'+'land-LENS').quantile(dim='ensemble',q=0.05)
                a2= global_land_mean.get(ex+'~'+feature+'~'+'land-LENS').quantile(dim='ensemble',q=0.95)
    ...
    if ex == 'dry_to_wet':
        fig.savefig(basic_dir+"code_whiplash/5-2.Figures/Fig.2.png",dpi=1500, bbox_inches='tight')
        fig.savefig(basic_dir+"code_whiplash/5-2.Figures/Fig.2.pdf",dpi=1500, bbox_inches='tight') 
    else:
        fig.savefig(basic_dir+"code_whiplash/5-2.Figures/Fig.S9.png",dpi=1500, bbox_inches='tight')
        fig.savefig(basic_dir+"code_whiplash/5-2.Figures/Fig.S9.pdf",dpi=1500, bbox_inches='tight') 
