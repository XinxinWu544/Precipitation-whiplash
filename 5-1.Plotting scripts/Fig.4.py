...
global_change = np.load(basic_dir+'code_whiplash/4-2.Input data for plotting/4.prec_and_whiplash_regimes.npy',allow_pickle=True).tolist()
...
for ex in ['dry_to_wet','wet_to_dry']:
    ...
    cycle_regime, cycle_lon = add_cyclic_point(d, coord=a1.lon)
    ...
    if ex =='wet_to_dry':
        cf_a1 = ax.contourf(a1.lon,a1.lat,a1,
                     ...
                     #extend='both',
                     colors= col[[0,1,2]])
        cf_a2 = ax.contourf(a2.lon,a2.lat,a2,
                     transform=ccrs.PlateCarree(),
                     levels=np.array(color_limit)[[3,4,5,6]],
                     #extend='both',
                     colors= col[[3,4,5]])
        cf_a3 = ax.contourf(a3.lon,a3.lat,a3,
                     transform=ccrs.PlateCarree(),
                     levels=np.array(color_limit)[[6,7,8,9]],
                     #extend='both',
                     colors= col[[6,7,8]])
        cf_a4 = ax.contourf(a4.lon,a4.lat,a4,
                     transform=ccrs.PlateCarree(),
