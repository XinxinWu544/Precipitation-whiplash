...
IF_forcings = np.load(basic_dir +'code_whiplash/4-2.Input data for plotting/5_S16-17.IF_forcings_distribution.npy',allow_pickle=True).tolist()
#sig_student = np.load(basic_dir + 'code_new/11-1.LENS_SF_global_and_region_stats/student_t_test_of_forcings.npy',allow_pickle=True).tolist()
If_sig_agreement = np.load(basic_dir + 'code_whiplash/4-2.Input data for plotting/5_S16-17.If_sig_agreement.npy',allow_pickle=True).tolist()
forcing_mean = np.load(basic_dir + 'code_whiplash/4-2.Input data for plotting/5_S16-17.LENS_and_SF_events_global_and_region_mean.npy',allow_pickle=True).tolist()
IF_forcings_global_and_regional_mean = np.load(basic_dir + 'code_whiplash/4-2.Input data for plotting/5_S16-17.IF_forcings_global_and_regional_mean.npy',allow_pickle=True).tolist()
...
for feature in ['frequency']:
    ...
    for ex in ['dry_to_wet','wet_to_dry']:
        ...
        for x in [0,1]:
            ...
            print(forcing[x])
            print(m.values)
            print(q.max().values - m.values)
            print(q.min().values - m.values)
...
for feature in ['frequency']:
    ...
    for ex in ['dry_to_wet','wet_to_dry']:
        ...
        ax1.hlines(xmin=-0.5,xmax=2.5,y=0,color='grey',linestyle='--',linewidth=0.6 )
        ...
        for x in [0,1]:
            ...
            err[0] = m.values - q.min()
            err[1] = q.max() - m.values
        ...
        for x in [0,1,2]:
            ...
            #plt.legend() 
            if p_num ==3:
                ax1.set_ylabel('Relative influence (%)',fontsize=7,labelpad=1)
        ...
        for x in [0,1]:
            ...
            Colors_limits=np.arange(-30,31,4)
            ...
            print(p_num)
            print('map!')
            ax.set_title(r"$\bf{("+ title_aux[p_num] +") }$ Relative influence of "+forcing[x]+' ('+ex_name+")",
                             pad=2, size=8.5, loc='left')
            lon=rif.lon
            lat=rif.lat
            ...
            vars()['first_year_LENS_negative_' +ex+'_'+feature] = pd.read_csv (basic_dir + 'code_whiplash/4-2.Input data for plotting/5_S16-17.'+'first_year_LENS_negative_' +ex+'_'+'feature.csv')
            vars()['first_year_LENS_positive_' +ex+'_'+feature] = pd.read_csv (basic_dir + 'code_whiplash/4-2.Input data for plotting/5_S16-17.'+'first_year_LENS_positive_' +ex+'_'+'feature.csv')
...
for feature in ['duration']:
    ...
    for ex in ['dry_to_wet','wet_to_dry']:
        ...
        xticks = [1930,1970,2010,2050,2090]
            #y_ = np.arange(0,101,20)
        ax.set_xticks(xtfile:///E:/research/4.East_Asia/Again/code_whiplash/4-1.Data-to-csv scripts/1_S8.climatology_of_features_and_time.pyicks)
        ax.set_xticklabels(xticks,fontsize=7)
        ...
        for x in [0,1]:
            ...
            #%
            Colors_limits=[-1.6,-1.2,-0.8,-0.4,-0.2,0,0.2,0.4,0.8,1.2,1.6,2,2.4]
            Colors_limits=np.arange(-3.5,4.5,0.5)
...
for feature in ['frequency']:
    ...
    for ex in ['dry_to_wet','wet_to_dry']:
        ...
        for ms in range(6):
            ...
            if p_num ==8:
                ax1.set_ylabel('Relative influence (%)',fontsize=8,labelpad=1)
